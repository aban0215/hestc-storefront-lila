...
export function Pagination({
  ...
  const renderPageButton = (
      ...
      <button
          key={p}
          className={clx("txt-xlarge-plus text-ui-fg-muted px-2", {
  ...
  const renderEllipsis = (key: string) => (
      <span
          key={key}
          className="txt-xlarge-plus text-ui-fg-muted items-center cursor-default px-1"
      >
      ...
    </span>
  )
  ...
  return (
      <div className="flex justify-center w-full mt-12">
        <div className="flex gap-4 items-center" data-testid={dataTestid}>

          {/* --- 上一页按钮 --- */}
          <button
              className={clx("txt-xlarge-plus transition-colors", {
                "text-ui-fg-muted cursor-not-allowed opacity-50": page <= 1,
          ...
          <button
              className={clx("txt-xlarge-plus transition-colors", {
                "text-ui-fg-muted cursor-not-allowed opacity-50": page >= totalPages,
